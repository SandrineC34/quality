...
from app.database.connection import non_conformites_collection, get_next_sequence
from app.actions.models.non_conformite import NonConformiteCreate, NonConformiteUpdate
...
def serialize(doc: dict) -> dict:
    ...
    Important : les champs date (y compris "date_cloture" et "date_action_curative") sont stockés en datetime() (bson l'exige), mais <input type="date">
    côté frontend n'accepte QUE le format "YYYY-MM-DD". On force donc
    systématiquement la partie date pure ici.
    """
    doc["_id"] = str(doc["_id"])
    for field in ("date", "date_cloture", "date_action_curative"):
        value = doc.get(field)
    ...
    for action in doc.get("actions_correctives") or []:
        for date_field in ("date_cloture", "date_objectif"):
            value = action.get(date_field)
            if isinstance(value, datetime):
                action[date_field] = value.date().isoformat()
            elif isinstance(value, date):
                action[date_field] = value.isoformat()
